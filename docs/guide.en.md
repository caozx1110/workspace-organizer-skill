...
A confirmed install normally reports `installed`. If reporting is interrupted
immediately after the atomic rename, or parent-directory durability cannot be
confirmed, the installer reconstructs state from both directory entries and
their identities. It reports `installed-with-durability-warning` only when the
exact canonical destination is visible and the staging entry is absent. This is
a successful install that should be checked again after a system restart.
